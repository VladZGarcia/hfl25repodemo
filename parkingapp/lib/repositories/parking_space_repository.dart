...
class ParkingSpaceRepository {
  ...
  Future<Parkingspace> addParkingSpace(Parkingspace parkingspace) async {
    final url = Uri.parse('http://10.0.2.2:8080/parking_spaces');
  ...
  Future<Parkingspace?> getById(String spaceId) async {
    final url = Uri.parse('http://10.0.2.2:8080/parking_spaces/$spaceId');
    Response response = await http.get(
      url,
      headers: {
        'Content-Type': 'application/json',
  ...
  Future<Parkingspace> update(Parkingspace parkingspace) async {
    final url = Uri.parse('http://10.0.2.2:8080/parking_spaces/${parkingspace.id}');
  ...
  Future<Parkingspace?> delete(String id) async {
    final url = Uri.parse('http://10.0.2.2:8080/parking_spaces/$id');
